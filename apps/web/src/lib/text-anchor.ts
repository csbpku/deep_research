/** Browser-safe SHA-256 helper for editor selection anchors. */
export async function sha256Hex(value: string): Promise<string> {
  ...
  // editor still needs the same stable content identity there, so use a
  // small standards-based fallback instead of failing selection and save
  // flows. It operates on the exact UTF-8 bytes used by Web Crypto.
...
function sha256BytesHex(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + bigSigma1 + choose + roundConstants[index]! + words[index]!) >>> 0;
