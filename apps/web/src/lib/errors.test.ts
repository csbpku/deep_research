...
describe('ERROR_HTTP_STATUS mapping', () => {
  ...
  it('matches contracts/error-codes.md documented statuses', () => {
    ...
    expect(ERROR_HTTP_STATUS[ERROR_CODES.AUTH_INVITE_INVALID]).toBe(403);
    expect(ERROR_HTTP_STATUS[ERROR_CODES.AUTH_REGISTRATION_DISABLED]).toBe(403);
    ...
    expect(ERROR_HTTP_STATUS[ERROR_CODES.URL_FETCH_DNS]).toBe(502);
    expect(ERROR_HTTP_STATUS[ERROR_CODES.URL_REDIRECT_LIMIT]).toBe(502);
    expect(ERROR_HTTP_STATUS[ERROR_CODES.UPSTREAM_RATE_LIMITED]).toBe(429);
