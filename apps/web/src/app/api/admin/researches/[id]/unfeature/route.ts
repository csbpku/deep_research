// BFF handler: POST /api/admin/researches/[id]/unfeature — Admin 取消精华。
...
>(async (req, ctx) => {
  ...
  const result = await prisma.$transaction((tx) =>
    setResearchFeatured(tx, {
      ...
      featured: false,
      adminAction: {
        action: ADMIN_RESEARCH_ACTIONS.UNFEATURE,
        requestId: actionRequestId,
      },
    }),
  );

  log.info('admin.research.unfeature', 'research unfeatured', {
    requestId,
    userId: u.id,
