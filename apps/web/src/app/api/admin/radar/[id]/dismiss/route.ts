// BFF handler: POST /api/admin/radar/[id]/dismiss — Admin 忽略雷达候选。
//
// 行为：requireAdmin → 校验来源 → $transaction(update status='rejected' +
// write admin_action)。
...
export const POST = apiHandler<[NextRequest, { params: Promise<{ id: string }> }]>(async (req, ctx) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    const updated = await tx.summary.update({
      ...
      data: { status: SUMMARY_STATUS.REJECTED },
      select: { id: true, status: true, updatedAt: true },
    });
    await writeAdminAction(tx, {
      actorId: u.id,
      action: ADMIN_RADAR_ACTIONS.DISMISS,
  ...
  log.info('admin.radar.dismiss', 'candidate dismissed', {
    requestId,
    userId: u.id,
