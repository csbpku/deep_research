...
export function RadarArticleHighlights({ summary, highlights, keyQuote, showSummary = true }: Props) {
  return (
    <section className="my-5 rounded-lg bg-muted/30 px-4 py-4" aria-labelledby="article-highlights-title">
      ...
      {showSummary && summary ? <p className="mb-3 text-sm leading-6 text-foreground/90">{summary}</p> : null}
      {highlights.length > 0 ? (
