...
import { BookOpenCheck, Compass, Loader2, Sparkles, Tag } from 'lucide-react';
  ...
  return (
    <div className="space-y-3 rounded-lg border border-border bg-card p-3 text-xs">
      <header className="flex items-center justify-between">
        ...
        <Badge variant={plan.brief.objective === 'explore' ? 'secondary' : 'default'} className="text-[10px]">
      ...
      {topic ? (
        <div className="flex items-center gap-1.5 rounded-md border border-primary/30 bg-primary/8 px-2 py-1.5 text-[11px] text-primary">
          <Compass className="size-3" />
          匹配专题：<span className="font-medium">{topic.name}</span>
          <span className="ml-auto text-muted-foreground">置信 {Math.round(topic.confidence * 100)}%</span>
        </div>
      ) : null}
      {plan.plan ? (
        <div className="rounded-md border border-border bg-muted/40 px-2 py-1.5">
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">预估 {plan.plan.estimatedMinutes} 分钟</p>
        </div>
      ) : null}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    accepted ? 'border-primary bg-primary/8' : 'border-border bg-card hover:bg-muted/60',
                  )}
                >
                  ...
                  <span className="mt-0.5 line-clamp-2 text-[11px] text-muted-foreground">{item.snippet}</span>
                </button>
              );
            })}
          </div>
      ...
      {plan.missingFields.length > 0 ? (
        <div className="rounded-md border border-amber-300/40 bg-amber-100/40 px-2 py-1.5 text-[11px] text-amber-800">
