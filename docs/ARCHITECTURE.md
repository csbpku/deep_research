...
**6.1.4 AI 追问内容的可见性（v3.3.1 重要澄清）**：

> **AI 追问（question + answer）是 session-private 的，**只有"点回评论区"才会变成 public。

| 行为 | 写回位置 | 可见性 |
|---|---|---|
| 读者问 AI "50 并发会爆吗" → AI 回答 | **AI 追问 session**（不写任何表，session 关闭就清） | **仅当前提问者** |
| 读者点 [💬 写回评论区] | `comments` 表 | **全员可见** |
| 作者在修改模式问 AI → AI 回答 | **修改 session 临时状态** | **仅作者**（不写表） |
| 作者点 [✍️ 应用回此版本] | `research_audit.prev_snapshot + diff` | **仅作者可见 diff**（published 内容更新后全员可见） |

> **核心原则：AI 追问是"私聊"体验，评论是"群聊"体验**。两者物理隔离：
> - AI 追问存在内存 / session 存储（无 DB 落盘）
> - 评论写 `comments` 表
> - **AI 追问内容永远不会进全文搜索**（防泄露半成品对话）
> - **session 关闭后追问历史自动清除**（除非显式"写回评论区"）

> **为什么 AI 追问不默认 public**：
> 1. 用户的疑问可能是错的、半生不熟的、不想让同事看到"我居然问这个"
> 2. AI 答案可能错（v3.2 原则"AI 必须人工把关"）——直接公开会污染团队认知
> 3. 与评论混淆：评论是"我深思熟虑后的话"，AI 追问是"我临时起意的问题"，两者质量天差地别
> 4. 写回评论区是显式动作——用户**主动决定**这条值得公开

**6.1.4.1 AI 追问 UI 不放"写回评论区"按钮（v3.3.1 新增）**：

> **AI 追问抽屉里不能有"写回评论区"按钮**。用户应**手动**去评论区手打。

**原因**：
- **诱导效应**：UI 上一键点"写回评论区"会让人**不思考就把 AI 半成品公开**，违反"AI 追问是私聊"原则
- **质量过滤**：评论应是用户**消化 AI 回答后自己写的话**，不是 AI 回答的复制粘贴
- **私聊 ≠ 群聊**：drawer 永远不应有"通向评论区"的快捷路径

**正确流程**（用户想公开 AI 答案时）：
```
[drawer 里问 AI] → AI 回答 (私聊)
        ↓ 关掉 drawer
[卡上 💬 评论] → 自己消化后手打
        ↓
发到 comments 表 (群聊)
...
> 抽屉里只允许 3 个操作：**📋 复制**、**🔄 重新生成**、**关闭**。复制后用户自己粘贴到评论区 = 主动公开。

**数据流示意**：

```
[用户问 AI]    ──→  [AI session 内存]    ──→  [关抽屉 = 清]
                     │                            ↑
                     │ (无写回评论按钮)           │ 普通关闭
                     ↓                            │
                  (无落地动作)  ─────────────────┘
