...
if [[ "$MODE" == "vps" ]]; then
  ...
  cp infra/certbot-renew.sh "$DEPLOY_DIR/infra/"
  cp infra/docker-compose.certbot.yml "$DEPLOY_DIR/infra/"
  cp infra/certbot.Dockerfile "$DEPLOY_DIR/infra/"
  ...
  mkdir -p "$DEPLOY_DIR/infra/certbot/www"
  touch "$DEPLOY_DIR/infra/certbot/www/.gitkeep"
  chmod +x "$DEPLOY_DIR/infra/certbot-renew.sh"
...
VPSSCRIPT
  ...
  echo ""
  echo "  3. If using TLS:"
  echo "     Point the domain A record to this VPS, then issue an ACME certificate:"
  echo -e "     ${CYAN}docker compose -f infra/docker-compose.yml --env-file .env up -d nginx${NC}"
  echo -e "     ${CYAN}docker compose -f infra/docker-compose.yml --env-file .env --profile certbot run --rm certbot certonly --webroot -w /var/www/certbot -d \"${VPS_DOMAIN:-example.com}\" --email you@example.com --agree-tos --no-eff-email${NC}"
  echo "     Set NGINX_CONFIG=nginx-tls.conf and NEXTAUTH_URL=https://${VPS_DOMAIN:-example.com} in .env, then:"
  echo -e "     ${CYAN}docker compose -f infra/docker-compose.yml --env-file .env up -d --force-recreate nginx${NC}"
  echo "     For renewals, run infra/certbot-renew.sh from the deployment directory."
  echo ""
