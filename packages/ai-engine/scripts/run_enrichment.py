    ...
    async with store.pool.connection() as conn:
        rows = await (
            await conn.execute(
                ...
                'OR CASE WHEN NOT ("originalMeta"->\'zread\' ? \'missingPages\') THEN FALSE '
                'WHEN jsonb_typeof("originalMeta"->\'zread\'->\'missingPages\') = \'array\' '
                'THEN jsonb_array_length("originalMeta"->\'zread\'->\'missingPages\') > 0 '
                'ELSE TRUE END'
                ')) '
