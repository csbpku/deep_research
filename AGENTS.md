...
- As of 2026-09-10: Weeks 1–13 / P1 / cognition-loop V2 已交付；当前继续收口 AI 调研产物、深度研究、雷达正文质量和全站体验。`main` 已包含 Hugging Face 可配置请求源、Zread 限流恢复、enrichment 限流隔离、render-review sidecar 和 HTTP UUID 兼容；本批代码尚未完成 VPS 发布实跑，最新事实以 [`docs/PROJECT_STATUS.md`](./docs/PROJECT_STATUS.md) 顶部条目为准。
...
- 生产 Compose 目标为 PostgreSQL、Web、AI engine、render-review、nginx 五个服务；GHCR 部署按同一 commit SHA 发布三个应用镜像，render-review 负责真实 Chromium 页面审核，AI engine 通过 HTTP sidecar 调用它。
- 本地运行：`launchd` 模板（`infra/launchd/`）支持常驻 AI engine（uvicorn 直接运行，无 `--reload`）和 Next.js 服务。
- **本次收口门禁**：`git diff --check`、`pnpm typecheck`、目标 Python 测试 `92 passed`、目标 Python `ruff`、目标 Python `mypy`、registry Compose config 均通过；Web/Python 全量测试和真实外部服务验收以 PROJECT_STATUS 的逐条记录为准，不把未在本次收口重跑的数量写成新基线。
